...
def run(cfg: DictConfig):
    ...
    for fold in range(n_splits):
        ...
        model = LitModel_for_Macro_Imu(cfg, num_classes=8, len_move=dm.len_move, 
                                       len_perform=dm.len_perform, class_weight=dm.class_weight)
        ...
        with torch.no_grad():
            ...
            model = LitModel_for_Macro_Imu.load_from_checkpoint(
                    checkpoint_path=os.path.join(ckpt_dir, f"best_of_fold_imu_{fold+1}.ckpt"),
                    cfg=cfg,
                    num_classes=8,
                    len_move=dm.len_move,
                    len_perform=dm.len_perform,
            ...
            for batch in dm.val_dataloader_imu():
                ...
                for pred, y_true in zip(y_pred, y):
                    ...
                    solution.append(gesture_classes[y_true.cpu().numpy()])
