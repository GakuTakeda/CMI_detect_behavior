...
from lit_model     import LitGestureClassifier_for_tof
import json
import os
import torch
import numpy as np
...
def run(cfg: DictConfig):
    ...
    for fold in range(n_splits):
        ...
        model = LitGestureClassifier_for_tof(cfg,
                                     dm.imu_ch, 5,
                                     dm.num_classes,
        ...
        trainer.fit(model=model, train_dataloaders=dm.train_dataloader_img(), val_dataloaders=dm.val_dataloader_img())
        trainer = L.Trainer(
            max_epochs      = cfg.train.epochs,
            accelerator     = cfg.train.device,
            callbacks = [
        ...
        with torch.no_grad():
            model = LitGestureClassifier_for_tof.load_from_checkpoint(
            ...
            submission = []
            solution = []
            for batch in dm.val_dataloader_img():
